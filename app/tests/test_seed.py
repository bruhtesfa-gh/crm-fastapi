...
from app.tests.utils.db import engine, get_test_db
    ...
    async for session in get_test_db():
        ...
        for permission in permissions:
            permission = Permission(
                name=permission, description="Permission to " + permission
            )
            session.add(permission)

        await session.commit()

        admin_permissions = await session.execute(
            select(Permission).where(
                Permission.name.ilike(f"%users%") | Permission.name.ilike(f"%roles%")
            )
        )
        admin_permissions = admin_permissions.scalars().all()
        print(jsonable_encoder(admin_permissions))
        manager_permissions = await session.execute(
            select(Permission).where(
        ...
        seals_permissions = await session.execute(
            select(Permission).where(
                Permission.name.ilike(f"%leads%")
                | Permission.name.ilike(f"%quotations%")
            )
        ...
        all_permissions = await session.execute(
            select(Permission).where(Permission.name.ilike(f"%audit-logs%"))
        )
        ...
        role_manager = Role(
            name="Manager", description="Manager with permission to approve quotations."
        )
        ...
        role_all = Role(
            name="All roles", description="All with permission to manage all."
        )
...
async def create_tables():
    ...
    async with engine.begin() as conn:
        ...
        await conn.run_sync(
            lambda sync_conn: Permission.metadata.create_all(bind=sync_conn)
        )
